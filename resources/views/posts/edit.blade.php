  ...
  <div class="w-full max-w-5xl bg-white rounded-lg shadow-md p-4 sm:p-6 md:p-8 relative">
    ...
    <form class="flex flex-col md:flex-row gap-6" action="{{ route('posts.update', $post->id) }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="w-full md:w-1/3 p-2">
        <label for="poster" class="cursor-pointer group block w-full h-48 sm:h-64 md:h-[400px] relative rounded-3xl overflow-hidden">
          <img id="preview" src="{{ asset('storage/' . $post->poster) }}" alt="poster"
            class="rounded-lg shadow-md w-full h-full object-cover group-hover:opacity-75 transition" />
      ...
      <div class="w-full md:flex-1 flex flex-col">
        ...
        <div class="mb-4">
          ...
          <textarea id="content" name="content" rows="5"
            class="resize-y w-full p-3 bg-gray-100 text-gray-900 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 break-words"
            placeholder="Contenido" required>{{ old('content', $post->content) }}</textarea>
        </div>
